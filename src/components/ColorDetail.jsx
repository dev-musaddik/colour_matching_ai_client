...
const ColorDetail = ({ color, onBack, setLoading }) => {
  ...
  const resizeImage = (file, maxDim = 1024, minQuality = 0.7) => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob(
          (blob) => {
            if (blob.size < 50 * 1024) resolve(file);
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      await uploadTrainingImage(color.id, resizedFile);
      fetchImages();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  return (
    ...
    >
      <button
        onClick={onBack}
        className="mb-6 text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:underline"
      >
      ...
      <div className="bg-white dark:bg-gray-800/50 p-6 rounded-2xl shadow-md mb-8 border border-gray-200 dark:border-gray-700">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Upload at least 3 images of this hair color to train the AI model. The
          more images you provide, the more accurate the model will be.
        </p>
        <button
          onClick={handleTrainClick}
          disabled={
            images.length < 3 || isTraining || color.status === "training"
          }
        ...
        >
          {isTraining || color.status === "training"
            ? "Training in Progress..."
            : `Train with ${images.length} Images`}
        </button>
      </div>
